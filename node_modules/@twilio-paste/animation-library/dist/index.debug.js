...
var createStringInterpolator$1 = (config2) => {
  if (!namedColorRegex)
  ...
  const interpolators = outputRanges.map((output2) => createInterpolator(__assign(__assign({}, config2), {output: output2})));
  return (input) => {
    let i = 0;
...
var useOnce = (effect) => import_react.useEffect(effect, emptyDeps);
var emptyDeps = [];
function useForceUpdate() {
  const update3 = import_react.useState()[1];
  const mounted = import_react.useState(makeMountedRef)[0];
...
function useMemoOne(getResult, inputs) {
  const [initial] = import_react.useState(() => ({
    inputs,
    result: getResult()
  }));
  const committed = import_react.useRef();
...
function usePrev(value) {
  const prevRef = import_react.useRef();
  import_react.useEffect(() => {
    prevRef.current = value;
  });
  return prevRef.current;
}
var useLayoutEffect = typeof window !== "undefined" && window.document && window.document.createElement ? import_react.useLayoutEffect : import_react.useEffect;
...
var withAnimated = (Component, host2) => {
  ...
  return import_react2.forwardRef((givenProps, givenRef) => {
    const instanceRef = import_react2.useRef(null);
    const ref = hasInstance && import_react2.useCallback((value) => {
    ...
    const observerRef = import_react2.useRef();
    useLayoutEffect(() => {
    ...
    import_react2.useEffect(callback, []);
    useOnce(() => () => {
      const observer2 = observerRef.current;
      each(observer2.deps, (dep) => removeFluidObserver(dep, observer2));
    });
    const usedProps = host2.getComponentProps(props.getValue());
    return /* @__PURE__ */ import_react2.createElement(Component, __assign(__assign({}, usedProps), {
      ref
    }));
  });
};
...
var linear = (t) => t;
var defaults = __assign(__assign({}, config.default), {
  mass: 1,
  damping: 1,
  easing: linear,
  clamp: false
...
function mergeConfig(config2, newConfig, defaultConfig) {
  if (defaultConfig) {
    defaultConfig = __assign({}, defaultConfig);
    sanitizeConfig(defaultConfig, newConfig);
    newConfig = __assign(__assign({}, defaultConfig), newConfig);
  }
...
function scheduleProps(callId, {key, props, defaultProps, state, actions}) {
  return new Promise((resolve, reject) => {
    ...
    function onStart() {
      ...
      try {
        actions.start(__assign(__assign({}, props), {callId, cancel}), resolve);
      } catch (err) {
        reject(err);
      }
    }
  ...
  return state.promise = (async () => {
    ...
    const animate = (arg1, arg2) => {
      ...
      return (async () => {
        ...
        const props2 = is.obj(arg1) ? __assign({}, arg1) : __assign(__assign({}, arg2), {to: arg1});
...
var SpringValue = class extends FrameValue {
  constructor(arg1, arg2) {
    ...
    if (!is.und(arg1) || !is.und(arg2)) {
      const props = is.obj(arg1) ? __assign({}, arg1) : __assign(__assign({}, arg2), {from: arg1});
  ...
  start(to2, arg2) {
    let queue;
    if (!is.und(to2)) {
      queue = [is.obj(to2) ? to2 : __assign(__assign({}, arg2), {to: to2})];
    } else {
      queue = this.queue || [];
      this.queue = [];
    }
  ...
  _merge(range, props, resolve) {
    ...
    if (!started) {
      ...
      if (!isEqual(config2.decay, decay) || !isEqual(config2.velocity, velocity)) {
        started = true;
      }
    }
...
function createLoopUpdate(props, loop2 = props.loop, to2 = props.to) {
  ...
  if (loopRet) {
    ...
    return createUpdate(__assign(__assign(__assign({}, props), {
      loop: loop2,
      default: false,
      pause: void 0,
...
var Controller = class {
  constructor(props, flush2) {
    ...
    if (props) {
      this.start(__assign({default: true}, props));
    }
  }
  get idle() {
...
function getSprings(ctrl, props) {
  const springs = __assign({}, ctrl.springs);
  if (props) {
    each(toArray(props), (props2) => {
      ...
      if (!is.obj(props2.to)) {
        props2 = __assign(__assign({}, props2), {to: void 0});
      }
...
var SpringContext = (_a) => {
  ...
  const inherited = import_react3.useContext(ctx);
  const pause = props.pause || !!inherited.pause, immediate = props.immediate || !!inherited.immediate;
  props = useMemoOne(() => ({pause, immediate}), [pause, immediate]);
  const {Provider} = ctx;
  return /* @__PURE__ */ import_react3.createElement(Provider, {
    value: props
  }, children);
};
...
function makeContext(target, init) {
  Object.assign(target, import_react3.createContext(init));
  target.Provider._context = target;
  target.Consumer._context = target;
  return target;
}
var SpringRef = class extends Function {
  constructor() {
...
function useSprings(length, props, deps) {
  ...
  if (propsFn && !deps)
    deps = [];
  const ref = import_react3.useMemo(() => propsFn || arguments.length == 3 ? new SpringRef() : void 0, []);
  const layoutId = import_react3.useRef(0);
  const forceUpdate = useForceUpdate();
  const state = import_react3.useMemo(() => ({
    ctrls: [],
    queue: [],
    flush(ctrl, updates2) {
  ...
  const context = import_react3.useContext(SpringContext);
  ...
  const values = springs.map((x) => __assign({}, x));
...
function useTransition(data, props, deps) {
  ...
  const ref = import_react3.useMemo(() => arguments.length == 3 ? new SpringRef() : void 0, []);
  const items = toArray(data);
  const transitions = [];
  const usedTransitions = import_react3.useRef(null);
  ...
  if (reused.length) {
    ...
    each(reused, (keyIndex, prevIndex) => {
      ...
      if (~keyIndex) {
        ...
        transitions[i] = __assign(__assign({}, t), {item: items[keyIndex]});
      } else if (props.leave) {
  ...
  each(transitions, (t, i) => {
    ...
    if (!to2.config) {
      const config2 = props.config || defaultProps.config;
      to2.config = callProp(config2, t.item, i, phase);
    }
    const payload = __assign(__assign(__assign({}, defaultProps), {
      delay: delay += trail,
      reset: false
    }), to2);
  ...
  const renderTransitions = (render) => /* @__PURE__ */ import_react3.createElement(import_react3.Fragment, null, transitions.map((t, i) => {
    const {springs} = changes.get(t) || t.ctrl;
    const elem = render(__assign({}, springs), t.item, t, i);
    return elem && elem.type ? /* @__PURE__ */ import_react3.createElement(elem.type, __assign(__assign({}, elem.props), {
...
function applyAnimatedValues(instance, props) {
  ...
  const {style, children, scrollTop, scrollLeft} = props, attributes = __rest(props, ["style", "children", "scrollTop", "scrollLeft"]);
...
var AnimatedStyle = class extends AnimatedObject {
  constructor(_a) {
    var {x, y, z} = _a, style = __rest(_a, ["x", "y", "z"]);
    const inputs = [];
    const transforms = [];
    if (x || y || z) {
...
var host = createHost(primitives, {
  ...
  getComponentProps: (_a) => {
    var {scrollTop, scrollLeft} = _a, props = __rest(_a, ["scrollTop", "scrollLeft"]);
    return props;
  }
});
